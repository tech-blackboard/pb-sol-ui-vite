...
test.describe('Website Contacts E2E Functionality', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    const contactsBtn = page.getByRole('button', { name: 'Contacts', exact: true });
  ...
  test('should validate required fields on Contact Form', async ({ page }) => {
    const addBtn = page.getByRole('button', { name: 'Add Contact', exact: true });
  ...
  test('should successfully submit Contact Request and view details', async ({ page }) => {
    ...
    await page.fill('input#phone', '9998887776');
    await page.locator('select#country').selectOption('United Kingdom');
    await page.locator('select#website_id').selectOption({ index: 1 });
    await page.fill('textarea#message', 'This is a test contact inquiry.');
    ...
    await expect(page.locator('h2:has-text("Contact Request Details")')).toBeVisible();
    ...
    await expect(page.locator('dt:text-is("Phone") + dd')).toHaveText('9998887776');
    await expect(page.locator('dt:text-is("Country") + dd')).toHaveText('United Kingdom');
    await expect(page.locator('h3:has-text("Message") + div')).toHaveText('This is a test contact inquiry.');

    const closeBtn = page.locator('button:has-text("Close")');
    await closeBtn.click();
  });

  test('should verify contact filter drawer inputs, combined filters, sorting, and reset button', async ({ page }) => {
    // 1. Initial count
    ...
    await expect(page.locator('h3:has-text("Contact Filters")')).toBeVisible();

    // 3. Test individual field filtering (Name field)
    await page.fill('input[placeholder="Name"]', 'E2E Contact');
    
    // Intercept search API call
    const searchPromise = page.waitForResponse(res => res.url().includes('/api/contact/search') && res.status() === 200);
    ...
    // Reset filters
    await filterBtn.click();
    const resetPromise = page.waitForResponse(res => res.url().includes('/api/contact/search') && res.status() === 200);
    ...
    const webPromise = page.waitForResponse(res => res.url().includes('/api/contact/search') && res.status() === 200);
    await page.locator('button:has-text("Apply")').click();
    await webPromise;

    // Reset
    await filterBtn.click();
    const resetPromise2 = page.waitForResponse(res => res.url().includes('/api/contact/search') && res.status() === 200);
    await page.locator('button:has-text("Reset")').click();
    await resetPromise2;
    await page.locator('button:has-text("✕")').click();
    ...
    const sortPromise = page.waitForResponse(res => res.url().includes('/api/contact/search') && res.status() === 200);
    await page.locator('button:has-text("Apply")').click();
    await sortPromise;
    ...
    // Reset
    await filterBtn.click();
    const resetPromise3 = page.waitForResponse(res => res.url().includes('/api/contact/search') && res.status() === 200);
    await page.locator('button:has-text("Reset")').click();
    await resetPromise3;
    await page.locator('button:has-text("✕")').click();
    ...
    await page.fill('input[placeholder="Country"]', 'United Kingdom');
    const combinedPromise = page.waitForResponse(res => res.url().includes('/api/contact/search') && res.status() === 200);
    await page.locator('button:has-text("Apply")').click();
    await combinedPromise;

    // Reset to default
    await filterBtn.click();
    const resetPromise4 = page.waitForResponse(res => res.url().includes('/api/contact/search') && res.status() === 200);
    await page.locator('button:has-text("Reset")').click();
    await resetPromise4;
    await page.locator('button:has-text("✕")').click();
    await expect(page.locator('table tbody tr')).toHaveCount(initialCount);
