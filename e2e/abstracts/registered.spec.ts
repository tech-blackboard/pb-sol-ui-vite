...
test.describe('Abstracts Registered E2E Functionality', () => {
  ...
  test('should support validation of payment receipt form and positive receipt generation in Registered status', async ({ page }) => {
    const nameText = `E2E Registered User ${Date.now()}`;
    ...
    await page.fill('input[id="Abstract Title"]', 'E2E Registered test research');
    ...
    const paymentReceiptBtn = page.locator('button:has-text("Payment Receipt")');
    await expect(paymentReceiptBtn).toBeVisible();

    // --- SUB-TEST 1: Generate Payment Receipt Form Error Validation ---
    await paymentReceiptBtn.click();
    const receiptModal = page.locator('div.relative.w-full.max-w-4xl');
    await expect(receiptModal).toBeVisible();

    // Preview without selecting option or filling required fields
    await page.locator('button:has-text("Preview Payment Receipt")').click();

    // Verify validation errors and red border
    await expect(receiptModal).toHaveClass(/border-red-500/);
    await expect(receiptModal.locator('text=Please select an option')).toBeVisible();
    await expect(receiptModal.locator('text=Registration fee is required and must be greater than 0')).toBeVisible();
    ...
    // Toggle accommodation (Note: ID is lookingForAccommodation in PaymentReceiptForm)
    const accommodationCheckbox = page.locator('input#lookingForAccommodation');
    await accommodationCheckbox.check();

    // Preview to trigger accommodation error validations
    await page.locator('button:has-text("Preview Payment Receipt")').click();
    await expect(receiptModal.locator('text=Check-in date is required')).toBeVisible();
    await expect(receiptModal.locator('text=Check-out date is required')).toBeVisible();
    await expect(receiptModal.locator('text=Accommodation fee is required and must be greater than 0')).toBeVisible();
    ...
    await page.locator('button:has-text("Preview Payment Receipt")').click();
    await expect(receiptModal.locator('text=Check-out must be after check-in date')).toBeVisible();
    ...
    const grandTotalLabel = page.locator('td:has-text("Grand Total") + td').or(page.locator('td:has-text("Total Price:") + td'));
    await expect(grandTotalLabel.first()).toHaveText('$1151');
    ...
    // Submit Receipt in Preview
    await page.locator('button:has-text("Send Payment Receipt")').or(page.locator('button:has-text("Send Receipt")')).first().click();
    
    // Confirm Send Receipt and handle API response
    const sendReceiptPromise = page.waitForResponse(
      res => res.url().includes('/send-payment-receipt')
    );
    await page.locator('button:has-text("Confirm & Send")').click();
    const sendReceiptResponse = await sendReceiptPromise;
    const sendReceiptStatus = sendReceiptResponse.status();
    const sendReceiptBody = await sendReceiptResponse.json();

    if (sendReceiptStatus === 200 || sendReceiptStatus === 201) {
      const expectedMsg = sendReceiptBody.message || 'Payment receipt sent successfully';
      await expect(page.getByText(expectedMsg).first()).toBeVisible();
      // Status updates to "Registered"
      await expect(page.locator('dt:has-text("Status") + dd span')).toHaveText('Registered');
    } else {
      const expectedError = Array.isArray(sendReceiptBody.message)
        ? sendReceiptBody.message[0]
        : (sendReceiptBody.message || 'Failed to send payment receipt');
      console.log(`E2E Verification - Send payment receipt failed. API Response error: "${expectedError}"`);
      if (sendReceiptStatus === 500) {
