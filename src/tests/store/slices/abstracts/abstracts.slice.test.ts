...
import { fetchAbstracts, updateStatusThunk, sendInvoiceThunk } from '../../../../store/slices/abstracts/abstracts.thunks'
  ...
  describe('abstracts slice', () => {
    ...
    it('should set selected abstract and modal status', () => {
      const abstract = { id: '1', status: { actionType: 'Approved' } }
    ...
    it('should clear selected abstract', () => {
      const state = reducer(
        { ...initialState, selected: { id: '1' }, modalStatus: 'Under Review' },
        clearSelected()
      )
    ...
    it('should handle fetchAbstracts.pending', () => {
      const state = reducer(initialState, fetchAbstracts.pending('', {} as any))
    ...
    it('should handle fetchAbstracts.fulfilled', () => {
      ...
      const state = reducer(
        initialState,
        fetchAbstracts.fulfilled(payload, '', {} as any)
      )
    ...
    it('should handle updateStatusThunk.fulfilled', () => {
      const startState = {
        ...initialState,
        rawItems: [{ id: '1', status: 'Old' }],
        items: [{ id: '1', status: 'Old' }],
      }
  
      const payload = { id: '1', status: { actionType: 'Approved' } }
  
      const state = reducer(
        startState,
        updateStatusThunk.fulfilled(payload as AbstractItem, '', {} as any)
      )
    ...
    it('should close invoice modal after sendInvoiceThunk.fulfilled', () => {
      ...
      const state = reducer(
        startState,
        sendInvoiceThunk.fulfilled({ success: true, message: 'Invoice sent', abstract: { id: '1', name: 'Test', email: 'test@test.com' } }, '', {} as any)
      )
