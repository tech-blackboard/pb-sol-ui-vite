...
interface Props {
    ...
    onClose: () => void
    onEdit?: (item: AccRegistrationItem) => void
    onDelete?: (item: AccRegistrationItem) => void
}

export default function AccRegistrationDetailsModal({ item, onClose, onEdit, onDelete }: Props) {
    if (!item) return null
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
            <div className="w-full max-w-3xl max-h-[90vh] rounded-lg bg-white shadow-2xl border border-gray-200 overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/50">
                    <h2 className="text-xl font-bold text-gray-800">Accommodation Details</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        <section className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                ...
                                <Field label="Institution" value={item.institution ? item.institution : '—'} />
                                <Field label="Country" value={item.country ? item.country : '—'} />
                            </div>
                        </section>

                        <section className="space-y-6">
                            <h3 className="text-sm font-semibold text-purple-600 uppercase tracking-wider border-b pb-2">Registration & Status</h3>
                            <div className="grid grid-cols-1 gap-4">
                                <Field label="Presentation" value={item.presentation ? item.presentation : '—'} />
                                <Field label="Participants" value={item.participants ? item.participants : '—'} />
                                <Field label="Reg Type" value={item.regtype ? item.regtype : '—'} />
                        ...
                        <section className="space-y-6 md:col-span-2">
                            <h3 className="text-sm font-semibold text-purple-600 uppercase tracking-wider border-b pb-2">Accommodation Details</h3>
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                                ...
                                <Field label="Accm" value={item.accm ?? '—'} />
                                <Field label="Accompng" value={item.acmpng ?? '—'} />
                                <Field label="Acc Price" value={item.acc_pr ? `$${item.acc_pr}` : '—'} />
                        ...
                        <section className="md:col-span-2 space-y-4">
                            <h3 className="text-sm font-semibold text-purple-600 uppercase tracking-wider border-b pb-2">Additional Context</h3>
                            <Field label="Alt Text" value={item.alt_text ? item.alt_text : '—'} />
                        </section>
                    </div>
